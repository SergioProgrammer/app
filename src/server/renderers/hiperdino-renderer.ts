...
const HIPERDINO_TEMPLATE_CANDIDATES = [
  path.join('public', 'Etiqueta-Hiperdino.pdf'),
  path.join('public', 'etiqueta-hiperdino.pdf'),
  path.join('public', 'Etiqueta_Hiperdino.pdf'),
]

const HIPERDINO_SPECIAL_TEMPLATE_MAP: Record<string, string> = {
  albahaca: 'albahacahiperdino.pdf',
  cebollino: 'cebollinohiperdino.pdf',
  cilantro: 'cilantrohiperdino.pdf',
  hierbahuerto: 'hierbahuertohiperdino.pdf',
  naranja: 'naranjahiperdino.pdf',
  perejil: 'perejilhiperdino.pdf',
  romero: 'romerohiperdino.pdf',
  rucula: 'ruculahiperdino.pdf',
}

const HIPERDINO_SPECIAL_LAYOUT = {
  albahaca: {
...
export interface HiperdinoRenderDeps {
...
export function resolveHiperdinoTemplatePath(customPath?: string | null, productName?: string | null): string | undefined {
  const productCandidates: string[] = []
  if (productName) {
    const key = normalizeTemplateKey(productName)
    if (HIPERDINO_SPECIAL_TEMPLATE_MAP[key]) {
      productCandidates.push(path.join(process.cwd(), 'public', HIPERDINO_SPECIAL_TEMPLATE_MAP[key]))
    }
    const suffixes = ['-hiperdino.pdf', '_hiperdino.pdf', '-hiperdino-template.pdf', '-hiperdino_etiqueta.pdf', 'hiperdino.pdf']
...
export async function renderHiperdinoLabel({
  fields,
  fileName,
  templatePath,
  shared,
  ...
  fileName: string
  templatePath?: string
  shared: HiperdinoRenderDeps
}): Promise<LabelRenderResult> {
  const preferredTemplate = resolveHiperdinoTemplatePath(templatePath, fields.productName)
  if (!preferredTemplate) {
    throw new Error('No encontramos plantilla específica para Hiperdino.')
  }
  ...
  const isNaranja = normalizedProductKey.includes('naranja')
  const hiperdinoLayout = HIPERDINO_SPECIAL_LAYOUT[normalizedProductKey as keyof typeof HIPERDINO_SPECIAL_LAYOUT]
  if (!hiperdinoLayout) {
    throw new Error('No hay layout definido para este producto Hiperdino.')
  }

  const weight = isNaranja ? null : shared.formatWeightText(fields.weight)
  const fechaEnvasado =
    shared.normalizeFieldValue(fields.fechaEnvasado, { formatAsDate: true }) ?? 'SIN FECHA'
  const loteHiperdino = formatHiperdinoLot(fields.lote, fields.fechaEnvasado, shared.formatLotText)

  const bodySize = Math.max(4.5, Math.min(5.5, pageWidth * 0.0038))
  const loteY = shared.mmToPageYDelta(hiperdinoLayout.loteYmmFromBottom, pageHeight)
  const leftX = shared.mmToPageX(hiperdinoLayout.loteXmm, pageWidth)
  const codeX = shared.mmToPageX(hiperdinoLayout.codeXmm, pageWidth)
  const codeY = shared.mmToPageYDelta(hiperdinoLayout.codeYmmFromBottom, pageHeight)

  page.drawText(loteHiperdino, {
    x: leftX,
    y: loteY,
    size: bodySize + 1,
    font: labelFont,
  ...
  if (!isNaranja && weight) {
    const pesoY = loteY + bodySize * hiperdinoLayout.pesoOffset
    const rightX = shared.mmToPageX(hiperdinoLayout.pesoXmm, pageWidth)
...
export function buildHiperdinoFallbackLines(
  fields: LabelRenderFields,
  shared: Pick<HiperdinoRenderDeps, 'normalizeFieldValue' | 'formatWeightText' | 'formatLotText'>,
): WhiteLabelLine[] {
  const normalizedProductKey = normalizeTemplateKey(fields.productName ?? '')
  const isNaranja = normalizedProductKey.includes('naranja')
  const lote = formatHiperdinoLot(fields.lote, fields.fechaEnvasado, shared.formatLotText)
  const fecha = shared.normalizeFieldValue(fields.fechaEnvasado, { formatAsDate: true }) ?? 'SIN FECHA'
  const baseLines: WhiteLabelLine[] = [
    { text: `LOTE: ${lote}` },
  ...
  return [...baseLines, { text: `PESO: ${peso}` }]
}

function formatHiperdinoLot(
  value?: string | null,
  fechaEnvasado?: string | null,
  formatLotText?: (v?: string | null) => string,
): string {
